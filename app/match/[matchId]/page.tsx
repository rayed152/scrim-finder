...
export default function MatchRoomPage({
  ...
  const endMatch = async () => {
    if (
      !confirm(
        "Are you sure you want to end this match? This will mark the scrim as completed for both teams.",
      )
    )
      return;
  ...
  useEffect(() => {
    if (!match || match.status === "completed" || match.isScheduled) return;
  ...
  }, [match?.status, match?.createdAt, match?.isScheduled]);
      ...
      <div className="max-w-6xl mx-auto z-10 relative">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
          <div>
            <Link
              href="/dashboard"
              className="text-sm text-neutral-500 hover:text-white transition-colors flex items-center gap-2 mb-4"
            >
          ...
          <div className="bg-neutral-900 border border-neutral-800 px-6 py-3 rounded-xl flex items-center gap-4">
            <div className="flex flex-col">
              ...
              <span
                className={`${match.status === "completed" ? "text-neutral-500" : "text-green-500"} font-bold uppercase text-sm`}
              >
            ...
            {match.status !== "completed" && !match.isScheduled && timeLeft && (
              <>
                <div className="w-px h-8 bg-neutral-800"></div>
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase text-neutral-500 font-bold tracking-widest">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <TeamCard
            team={match.team1}
            isMyTeam={match.team1.id === teamId}
            label="Team 1"
          />
          ...
          <TeamCard
            team={match.team2}
            isMyTeam={match.team2.id === teamId}
            label="Team 2"
          />
        </div>
        ...
        <div className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-8 backdrop-blur-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="space-y-3">
              <div className="w-10 h-10 bg-neutral-800 rounded-lg flex items-center justify-center text-red-500 font-bold">
                1
              </div>
              <h4 className="font-bold">Contact Opponent</h4>
              <p className="text-sm text-neutral-400">
                Add the opponent's captain on Discord to coordinate the custom
                game lobby.
              </p>
            </div>
            <div className="space-y-3">
              ...
              <p className="text-sm text-neutral-400">
                Settings: Tournament Mode, Cheats Off, Overtime: Win by Two.
                Server: {match.server}.
              </p>
            </div>
            <div className="space-y-3">
              ...
              <p className="text-sm text-neutral-400">
                Good luck! Once finished, both teams should report the score in
                the dashboard.
              </p>
            </div>
          </div>
...
function TeamCard({
  ...
  return (
    <div
      className={`rounded-2xl border ${isMyTeam ? "border-red-500/30" : "border-neutral-800"} overflow-hidden bg-neutral-900 shadow-xl`}
    >
      <div
        className={`p-6 border-b ${isMyTeam ? "border-red-500/20 bg-red-500/5" : "border-neutral-800 bg-neutral-900/50"}`}
      >
        <div className="flex justify-between items-center mb-1">
          <span
            className={`text-[10px] uppercase font-black tracking-[0.2em] ${isMyTeam ? "text-red-500" : "text-neutral-500"}`}
          >
            ...
            {team.members.map((member, i) => (
              ...
              >
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="text-white font-bold group-hover:text-red-400 transition-colors uppercase italic">
                      {member.user.valorantTag || member.user.name}
                    </span>
                    <span className="text-[10px] text-neutral-500">
                      Rank Score: {member.user.rank || "N/A"}
                    </span>
                  </div>
                </td>
